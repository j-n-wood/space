...
void DroneFleetMarkers::update(float delta)
{
    ...
    // update explosion markers: lifetime in z, remove expired
    for (auto &ep : explosion_pos)
    {
        ep.z += delta;
    }
    // Remove expired explosion markers
    explosion_pos.erase(std::remove_if(explosion_pos.begin(), explosion_pos.end(),
                                       [](const Vector3 &ep)
                                       { return ep.z >= 2.0f; }), // 2s lifetime
                        explosion_pos.end());
}
...
void DroneFleetMarkers::killRandom(int k)
{
    ...
    for (int j = 0; j < k; j++)
    {
        ...
        // explosion marker at the killed slot's last position (for the renderer).
        // use z for lifetime
        explosion_pos.push_back({slot_pos[scratch_idx[j]].x, slot_pos[scratch_idx[j]].y, 0.0f});
    }
    live_count -= k;
}
