...
Done. Each `Orrery` holds a `std::vector<LocationRender>` built parallel to
`system->locations` — screen position, screen radius, and the belt band's two radii. Draw,
hover, click and the transit lines all read it, so they cannot disagree about where a body is
or how big it is.

The table rebuilds in `Orrery::update()`, called from the owning page's `update()` and once
from `activate()` (page update runs after render). It is gated on a `last_update_time` stamp
against `game->game_time`, so a paused orrery does no position work at all. `setSystem()`,
zoom and `focusOnLocation()` reset the stamp, since those change the table's inputs without
the clock advancing.

Still open if it ever matters: a `resolved_position` cache on `Location` itself, filled by
`System::update`. That would let the two orreries share one parent walk — they would still
need a screen table each, since their transforms differ.
