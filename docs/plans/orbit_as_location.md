...
**Page titles and the header breadcrumb — delivered**, see
[view_focus.md](view_focus.md). `SublocationType` is gone (pages take
`LOCATION_TYPE_ORBIT` / `LOCATION_TYPE_SURFACE`, `Facility::sublocation()` is
`Location::inOrbit()`), and `ViewState` now separates the two questions that field
conflated: `getCurrentPlace()` is exactly where the focus is, `getCurrentBody()` is the body
whose facilities the sidebar offers. The header shows the place, so the titles need no
orbit/surface prefix:
